import { $nodeSchema, $view, $remark } from '@milkdown/kit/utils';
import type { MilkdownPlugin } from '@milkdown/kit/ctx';
import { createMermaidNodeView } from './mermaid-node-view';

// -- Remark plugin: transform code[lang=mermaid] into mermaidBlock AST nodes --
// This ensures our schema matches before Crepe's code block handler.

function visitNode(node: any, callback: (node: any, index: number, parent: any) => void, parent?: any, index?: number) {
  if (node.type === 'code') {
    callback(node, index ?? 0, parent);
  }
  if (node.children) {
    // Iterate in reverse to safely mutate during traversal
    for (let i = node.children.length - 1; i >= 0; i--) {
      visitNode(node.children[i], callback, node, i);
    }
  }
}

function remarkMermaid() {
  return (tree: any) => {
    visitNode(tree, (node, index, parent) => {
      if (node.lang === 'mermaid' && parent?.children) {
        parent.children[index] = {
          type: 'mermaidBlock',
          value: node.value,
        };
      }
    });
  };
}

export const remarkMermaidPlugin = $remark('remarkMermaid', () => remarkMermaid as any);

// -- Mermaid node schema --

export const mermaidId = 'mermaid';

export const mermaidSchema = $nodeSchema(mermaidId, () => ({
  group: 'block',
  atom: true,
  draggable: true,
  attrs: {
    value: { default: '' },
  },
  parseDOM: [
    {
      tag: `div[data-type="${mermaidId}"]`,
      getAttrs: (dom) => ({
        value: (dom as HTMLElement).dataset.value ?? '',
      }),
    },
  ],
  toDOM: (node) => {
    const div = document.createElement('div');
    div.dataset.type = mermaidId;
    div.dataset.value = node.attrs.value;
    div.textContent = node.attrs.value || '(mermaid)';
    return div;
  },
  parseMarkdown: {
    match: (node) => node.type === 'mermaidBlock',
    runner: (state, node, type) => {
      state.addNode(type, { value: node.value as string });
    },
  },
  toMarkdown: {
    match: (node) => node.type.name === mermaidId,
    runner: (state, node) => {
      state.addNode('code', undefined, node.attrs.value, {
        lang: 'mermaid',
      });
    },
  },
}));

// -- Node view --

export const mermaidView = $view(mermaidSchema.node, () =>
  createMermaidNodeView()
);

// -- Export all plugins --

export const mermaidPlugins: MilkdownPlugin[] = [
  ...remarkMermaidPlugin,
  mermaidSchema.node,
  mermaidSchema.ctx,
  mermaidView,
];
