...
* `Ctrl`+点击（macOS 为 `Cmd`+点击）任意链接，用系统默认浏览器打开。所见即所得模式与源码模式下都可用。
* 普通点击会弹出链接地址，点那个地址同样能打开浏览器。
* 协议可以省略 —— `[百度](www.baidu.com)` 会按 `https://www.baidu.com` 打开。

### 跳转文档链接

指向文档旁边、而非指向网络的链接，用同样的 `Ctrl`+点击（或点气泡里的地址）在编辑器内跳转：

* `./notes.md`、`../design/api.md`、`/abs/path/readme.md` —— 打开那个文件。`.md` 可以省略，`[设计](./design)` 会找到 `design.md`。
* `#小节标题` —— 滚到当前文档的那个标题。GitHub 风格的 slug 和标题原文都认，`#My Heading` 和 `#my-heading` 落到同一处。
* `./notes.md#小节` —— 打开文件**并**定位到标题。
* `./报告.pdf`、`./图表.png` —— 交给系统默认程序打开。

文件或标题不存在时会提示一句，界面不会动。

### 前进与后退

每一次文档切换 —— 链接跳转、点文件树、`Ctrl+O` —— 都会被记下，可以原路返回，并回到离开时的滚动位置：
...
输入 `$行内$` 或 `$$块级$$` 公式——顺序不限，先打好两侧的定界符再回到中间填内容同样有效。`$$` 加空格可新建空的块级公式；行前 `+` 按钮与 `/math` 里都能找到 **Math Block** 与 **Inline Math**。

新建的公式会直接拿到光标，方向键可以走出来。点击公式可用 MathLive 可视化编辑，并切换到 LaTeX 源码。块级公式里按 `Enter`（或虚拟键盘的回车）新增一行，并按 `=` 对齐。
...
* `Ctrl+\` 切换侧边栏（文件树 + 文档大纲）。文件树 tab 只在打开文件夹后才出现。拖动右边缘可调整宽度（会被记住），双击该边缘恢复默认。过长的文件名会截断显示，悬停可看到全名。
* 状态栏的 **`</>`** 按钮在所见即所得与 Markdown 源码之间切换。源码模式是完整的 CodeMirror 6 编辑器，带 Markdown 高亮、撤销历史、查找替换与多光标。切换模式不再清空撤销历史，`Ctrl+Z` 仍能退回到切换之前的编辑。
...
| 浏览器打开链接 | `Ctrl`+点击（或点气泡里的地址） |
| 跳转文档链接 | `Ctrl`+点击（或点气泡里的地址） |
| 后退 / 前进 | `Alt+←` / `Alt+→`（或鼠标侧键） |
