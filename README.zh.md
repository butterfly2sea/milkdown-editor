...
* **查找替换** — 支持正则、区分大小写、全词匹配，替换支持捕获分组

* **高亮** — `==高亮==` 标记，可从选中悬浮工具栏切换（纯 Markdown，不含 HTML）

* **编辑区缩放** — `Ctrl`+滚轮 或 `Ctrl`+`]` / `[` / `0` 缩放正文

* **图片本地化** — 粘贴或拖入图片，自动复制到 `<文件名>.assets/` 文件夹并以相对路径引用

* **文件管理** — 打开文件/文件夹、保存、自动保存（2 秒防抖）

* **导出** — 导出 HTML（内嵌样式）或 PDF（通过打印）

* **主题** — 明暗主题，平滑过渡

* **源码模式** — 在所见即所得与 Markdown 源码之间切换
...
* **选中工具栏** — 选中文字后可加粗 / 斜体 / 删除线 / 行内代码 / 链接 / **高亮**，以及复制 / 剪切 / 粘贴。
...
先保存文档（资源文件夹由文件路径决定）。然后：

* **粘贴或拖入图片** → 复制到文档旁的 `<文件名>.assets/` 文件夹，并以相对路径插入（`![](<文件名>.assets/img-….png)`）。
* **本地化已有图片** — 点击状态栏的图片按钮（或按 `Ctrl+Alt+I`），把网络图片下载、绝对路径图片复制到资源文件夹，并改写为相对路径。

图片通过 Tauri 的 asset 协议显示，磁盘上的 Markdown 保持相对路径、便于迁移。（仅桌面版应用。）
...
* `Ctrl+\` 切换侧边栏（文件树 + 文档大纲）。
* 状态栏的 **`</>`** 按钮在所见即所得与 Markdown 源码之间切换。
